...
class CmdVel2Gazebo:
    ...
    def __init__(self):
        ...
        self.pub_real = rospy.Publisher('/mk/cmd_vel',Twist,queue_size=1)  # convert msg to Twist format
	...
	#if(data.linear.x<0):
	#	self.z = -self.z    #backup inverse steer   smq 
        self.lastMsg = rospy.Time.now()
	# rospy.loginfo("[cmdvel2gazebo] data.z:%f",data.angular.z);
	# rospy.loginfo("[cmdvel2gazebo] self.z:%f",self.z);
    ...
    def publish(self):
        ...
        if msgs_too_old:
            ...
            #msg twist
            msgTwist = Twist()
            msgTwist.linear.x = msgRear.data
            msgTwist.linear.y = 0
            msgTwist.linear.z = 0
            msgTwist.angular.x = 0
            msgTwist.angular.y = 0
            msgTwist.angular.z = msgSteer.data	
            self.pub_real.publish(msgTwist)
        ...
        if self.z != 0:
            ...
            #msg twist
            msgTwist = Twist()
            msgTwist.linear.x = (msgRearL.data+msgRearR.data)/2.0
            msgTwist.linear.y = 0
            msgTwist.linear.z = 0
            msgTwist.angular.x = 0
            msgTwist.angular.y = 0
            msgTwist.angular.z = (msgSteerL.data+msgSteerR.data)/2.0	
            self.pub_real.publish(msgTwist)
